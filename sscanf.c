 ...
 */
static s64 strtoq(const char *nptr, char **endptr, int base) {
  const char *s = nptr;
  u64 acc;
  int unsign = 0;
  struct str_info *info;
  ...
  if (info->any < 0)
    ...
    acc = -acc;
  if (endptr != 0) *endptr = __DECONST(char *, info->any ? s - 1 : nptr);
...
static const u_char *__sccl(char *tab, const u_char *fmt) {
  ...
  for (n = 0; n < 256; n++) tab[n] = v; /* memset(tab, v, 256) */
...
int vsscanf(const char *inp, char const *fmt0, va_list ap) {
  ...
  for (;;) {
    ...
    if (isspace(c)) {
      while (inr > 0 && isspace(*inp)) nread++, inr--, inp++;
      continue;
    }
    if (c != '%') goto literal;
    width = 0;
  ...
  again:
    ...
    switch (c) {
      case '%':
      literal:
        if (inr <= 0) goto input_failure;
        if (*inp != c) goto match_failure;
        inr--, inp++;
        nread++;
        continue;
